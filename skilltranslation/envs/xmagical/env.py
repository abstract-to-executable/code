...
class SweepToTopEnv(BaseEnv):
    ...
    def set_state_xy(self, state):
        # Note that this does not set any angles
        self._robot.body._set_position(state[:2])
        for i, target_shape in enumerate(self.__debris_shapes):
            debris_xy = state[2 + 2 * i : 2 + 2 * (i + 1)]
            target_shape.shape_body._set_position(debris_xy)
    ...
    def score_on_end_of_traj(self) -> float:
        ...
        if len(overlap_ents) == 0:
            score = 0
        return score
    def get_done_debris(self) -> list:
        overlap_ents = self.__sensor_ref.get_overlapping_ents(
            contained=True, ent_index=self.__ent_index
        )
        done_debris = [False, False, False]
        for i, debris in enumerate(self.__debris_shapes):
            if debris in overlap_ents:
                done_debris[i] = True
        return done_debris
    ...
    def step(
        ...
        info['stats'] = dict(eval_score=self.score_on_end_of_traj())
        done = self.score_on_end_of_traj() > 0.99
        if self.use_state:
            obs = self.get_state()
...
if __name__ == "__main__":
    ...
    env: SweepToTopEnv = gym.make(f'SweepToTop-{embodiment}-State-Allo-TestLayout-v0')
    env.seed(0)
    state = env.reset()
    env: SweepToTopEnv = gym.make(f'SweepToTop-{embodiment}-State-Allo-Demo-v0')
    env.seed(0)
    env.reset()
    i = 0
    stime = time.time()
