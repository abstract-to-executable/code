...
class TrajectoryEnv(gym.Env):
    ...
    def step(self, action=...):
        ...
        # print('======== {:d} ======='.format(self.env_steps))
        # print('action:', action.sum())
        # print('obs:', obs['observation'].sum())
        # # if self.env_steps > 5:
        #     exit()
    ...
    def stack_obs(self, obs):
        ...
        # print(prepend_start, frame_start, step)
    ...
    def reset(self, init_plan_state=None, **kwargs):
        ...
        if self.stack_size > 1:
            self.stack_obs(obs)
        # print('########### init obs', obs["observation"].sum())
        if self.raw_obs_only == True:
            obs = obs["observation"]
        return obs
