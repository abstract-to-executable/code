...
from skilltranslation.envs.blockstacking.env import BlockStackFloatPandaEnv, BlockStackPandaEnv, BlockStackMagicPandaEnv
...
from skilltranslation.envs.world_building import add_target
from sapien.core import Pose

from skilltranslation.utils.pose_est import predict_pose, predict_pose_noseg
...
class BlockStackTrajectory(TrajectoryEnv):
    ...
    def reset(self, *args, **kwargs):
        ...
        # check panda hand qpos.q == teacher qpos.q
        teacher_pose_p = self.trajectory['observations'][0][0:3]
        for link in self.env.get_articulations()[0].get_links():
            if link.name == 'panda_hand':
                student_panda_hand_pos = link.pose
        # assert np.allclose(teacher_pose_p, student_panda_hand_pos.p,atol=1e-6 )
        # find goal coords from teacher traj
        teacher_goal_coords = self.trajectory['observations'][-1][-7:-4]
        student_goal_coords = self.env.goal_coords[0].flatten()
        # assert np.allclose(teacher_goal_coords, student_goal_coords, atol=5e-3)

        # check inital block pose
        teacher_block_init_pose = self.trajectory['observations'][0][-7: -4]
        student_block_init_pose = self.env.blocks[0].get_pose().p
        # assert np.allclose(teacher_block_init_pose, student_block_init_pose, atol=1e-7)
    ...
    def format_ret(self, obs, ret):
        if self.reward_type == "original" or self.reward_type == "dense":
            ...
            if self.improved_farthest_traj_step:
                ...
                reward += 10 * (1 - np.tanh(dist_to_next*30))
            else:
                if self.farthest_traj_step < len(self.teacher_student_coord_diff) - 1:
                    ...
                    reward += 10 # add huge reward if agent did reach end
            ...
            if self.improved_farthest_traj_step:
                look_ahead_idx = int(min(self.farthest_traj_step, len(self.teacher_student_coord_diff) - 1))
                dist_to_next = self.weighted_traj_diffs[look_ahead_idx]
                reward += 20 * (1 - np.tanh(dist_to_next*30))
            else:
                if self.farthest_traj_step < len(self.teacher_student_coord_diff) - 1:
                    reward -= 0 # add step wise penalty if agent hasn't reached the end yet
                else:
                    dist_to_next = self.weighted_traj_diffs[-1]
                    reward += 4 * (1- np.tanh(dist_to_next*30)) # add big reward if agent did reach end

            return reward
        elif self.reward_type == "lcs_dense2":
            reward = 0
            reward += ret / 5
            if self.improved_farthest_traj_step:
                look_ahead_idx = int(min(self.farthest_traj_step, len(self.teacher_student_coord_diff) - 1))
        ...
        if self.teacher_student_coord_diff[-1] < 0.05 and self.teacher_student_world_diff[-1] < max_world_diff: # needs to align with block stack planner threshold
            if "train" not in self.goal:
                ...
                if self.reset_agent_after_trajectory:
                    ...
                    print("RESET AGENT")
                    print("TEACHER PARt", self.abstract_part)
            ...
            if self.partial_trajectories:
                ...
                if len(traj['observations']) > 0:
                    ...
                    # print(f"SELCT TEACHER {self.abstract_part}")
                else:
                    self.abstract_part -= 1
    ...
    def get_trajectory(self, t_idx):
        ...
        # print(trajectory["observations"][:, -7:].shape)
        if self.sim_world_noise is not None:
...
if __name__ == "__main__":
    ...
    # env=BlockStackTrajectory(trajectories=['0'], trajectories_dataset="./datasets/blockstack/dataset.pkl",
    #                                  stack_size=5, max_trajectory_length=300)
    env=BlockStackTrajectory(trajectories=['0'], trajectories_dataset="./datasets/pick_and_place_silo/dataset.pkl",
                                     stack_size=5, max_trajectory_length=300, goal='pick_and_place_silo', num_blocks=1)
    env.seed(0)
    obs=env.reset()
    # print(obs.keys())
    #env.render()
    env.draw_teacher_trajectory(0)
    while True:
        env.render()
