...
from skilltranslation.envs.blockstacking.block_spawns import get_block_spawn_loc
from skilltranslation.utils.blockstack_utils.goal_state import *
from skilltranslation.utils.pose_est import estimate_single_block_pose_realsense
...
from skilltranslation.envs.blockstacking.base_env import PandaEnv, FloatPandaEnv
from skilltranslation.envs.blockstacking.agents.panda import  Panda
...
import skilltranslation.envs.blockstacking.test_configs as test_configs
        ...
        if len(self._fixtures) == 0:
            ...
            for i in range(max_trials):
                ...
                if np.all(dist > fixture_radius + min_radius) and np.all(dist < fixture_radius + max_radius):
                    # print(f"Found a valid sample at {i}-th trial - {dist} > {fixture_radius} + {min_radius}")
                    break
        if append:
...
class BlockStackPandaEnv(PandaEnv):
    ...
    def render(self, mode="human"):
        ...
        if mode == 'state_visual':
            ...
            cam.take_picture()
            # cam.take_picture()
            # view = read_images_from_camera(cam, depth=True, seg_indices=[0])
            # # print(view['seg'].shape)
            # # for name in view:
            # #     print(name, view[name].shape)
            # view['seg'] = view['seg'].reshape((128,128,1))
            # #print(view.keys())
            # return view
