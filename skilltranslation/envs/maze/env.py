...
from skilltranslation.envs.sapien_env import SapienEnv
from skilltranslation.envs.world_building import add_ball, add_target
...
class MazeEnv(SapienEnv):
    ...
    def _add_actors(self):
        ...
        if self.agent_type == "point":
            ...
            #     pose=sapien.Pose([0, 0, 0],  q=axangle2quat([0, 0, 1], np.deg2rad(90))),
            #     half_length=thickness*box_length, radius=thickness, color=[0.1,0.1,0.1])
            
            # builder.add_capsule_collision(
            #     pose=sapien.Pose([0, 0, 0],  q=axangle2quat([0, 0, 1], np.deg2rad(90))),
            #     half_length=thickness*box_length, radius=thickness, material=ball_mtl, density=density
            # )
            # builder.add_capsule_visual(pose=sapien.Pose([thickness*2, thickness*(box_length-1), 0],  q=axangle2quat([0, 0, 1], np.deg2rad(0))), half_length=thickness*0.5, radius=thickness, color=[0.9,0.1,0.1])
            # builder.add_capsule_collision(pose=sapien.Pose([thickness*2, thickness*(box_length-1), 0],  q=axangle2quat([0, 0, 1], np.deg2rad(0))), half_length=thickness*0.5, radius=thickness)
    ...
    def check_success(self):
        ...
        return done
    def step(self, action, allow_finish=True):
        """
    ...
    def _set_state(self, state):
        if self.agent_type == "rect":
            ...
            # TODO let scene settle? z-pos might not be accurate
        elif self.agent_type == "point":
            agent = state["agent"]
            self.agent.set_pose(sapien.Pose(
...
if __name__ == "__main__":
    np.random.seed(11)
    env = MazeEnv(random_init=False, agent_type="couch", world_size=50, max_walks=6, walk_dist_range=(12, 40), skip_map_gen=False, reward_type="dense2")
    env.seed(5)
    env.reset()
    env.render()
    viewer = env.viewer
    ...
    while steps < 100000:
        ...
        if done:
            break
    end_time = time.time()
    print(f"FPS: {steps / (end_time-start_time)}")
