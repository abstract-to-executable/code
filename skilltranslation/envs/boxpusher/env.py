...
import skilltranslation.envs.boxpusher.task_config as task_config
from skilltranslation.envs.sapien_env import SapienEnv
# from skilltranslation.envs.map_gen import generate_targets
...
class BoxPusherEnv(SapienEnv):
    ...
    def step(self, action=np.array([0, 0.0]), allow_finish=True):
        ...
        if done == False and finished_box:
            self.set_target()
        # old_xys = []
        # for idx, actor in enumerate(self.balls[1:]):
        #     # if self.balls_meta[idx+1]["done"]: continue
        #     old_xys.append(actor.get_pose().p[:2])
        self._scene.step()
        
        
        # pose = self.balls[controlled_ball].get_pose()
        # pose.set_q(q)
        # self.balls[controlled_ball].set_pose(pose)
        if self.task == 'silo_obstacle_push':
            q = axangle2quat([0, 0, 1], np.deg2rad(90))
            pose = self.balls[0].get_pose()
            # print(self.balls[0], pose)
            pose.set_q(q)
            # print(self.balls[0], pose)
            ...
            #     # if self.balls_meta[idx+1]["done"]: continue
                ...
                pose.set_q(q)
            #     new_p = pose.p.copy()
            #     zero_out_idx = np.argmin(np.abs(old_xys[idx] - pose.p[:2]))
            #     new_p[zero_out_idx] = old_xys[idx][zero_out_idx]
            #     pose.set_p(new_p)
                actor.set_pose(pose)
...
if __name__ == "__main__":
    np.random.seed(0)
    env = BoxPusherEnv(magic_control=False, balls=1, fixed_env=True, obs_mode="dense", speed_factor=0.5, task='silo_obstacle_push')
    env.seed(1)
    env.reset()
    env.reset()
    env.render()
    ...
    while True:
        ...
        # if steps < 5:
        #     action[0] = 3
        if env.viewer.window.key_down("r"):
