...
class TrainerTeacherStudent:
    ...
    def get_loss_from_batch(self, batch):
        ...
        if self.state_loss_fnc is not None:
            ...
            # TODO, add loss using predicted action and current state?
            state_dims = stacked_student_frames.shape[2] - act_dim
            # drop final frame prediction which is predicting on null action
            y_state = y["state"][:, :-1]
            y_state = y_state.reshape(-1, state_dims)[student_attn_mask[:, :-1].reshape(-1)]
            target_student_states = stacked_student_frames[:, 1:].reshape(-1, state_dims)[student_attn_mask[:, :-1].reshape(-1)]
