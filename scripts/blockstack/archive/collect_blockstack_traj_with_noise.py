...
def pick_and_place_magic_gripper(env:MultipleBlockStack, render=False):
    ...
    for j in range(len(magic_landmarks)):
        ...
        while True:
            ...
            if internal_clock == 2: print(env.get_blocks()[0].pose.p)
            if dist < 0.03 :
                if internal_clock == 1:
                    env.create_magic_drive_with(env.get_blocks()[int(j//6)])
                    print(f"connect at step {temp_clock} of phase{internal_clock}")
                    action[6] = -0.01
            ...
            noise = np.zeros(7)
            #noise[:3] = np.random.normal(0, 0.05, (3,)) * 0.015  # gaussian noise with mu=0, sigma=0.1, and scaled with
            action = np.clip(action, env.action_space.low, env.action_space.high)
            obs,_,_,_ = env.step(action + noise)
            ...
            if render:
                ...
                imgs.append(img)
            temp_clock+=1
        print(f"MAGIC: On block {int(j//6)} phase {internal_clock} takes {temp_clock} stpes ")
    magic_traj['observations'].append(env.get_obs())
    if render:
        return imgs
    return magic_traj
...
def pick_and_place_floating_gripper(env:MultipleBlockStack, render=False):
    ...
    for j in range(len(float_landmarks)):
        ...
        while True:
            ...
            noise = np.zeros(7)
            #noise[:3] = np.random.normal(0, 0.05, (3,)) * 0.015
            action = delta_qpos[:7].copy()
            action = np.clip(action, env.action_space.low, env.action_space.high)
            ...
            if render:
                ...
                imgs.append(img)
            temp_clock += 1
        print(f"FLOAT: On block {int(j//6)} phase {internal_clock} takes {temp_clock} stpes ")
...
def generate_dataset():
    ...
    j=0
    while osp.isdir(osp.join(dataset_path,'noisy_pick_and_place-' + str(j))):
        j+=1

    path=osp.join(dataset_path,'noisy_pick_and_place-' + str(j))
...
def test_and_and_render_heuristic_pick_and_place(to_check=[2]):
