...
if __name__ == "__main__":
    ...
    def make_env(idx):
            def _init():
                ...
                env = gym.make("BoxPusher-v0", obs_mode="dict", balls=1, magic_control=True, target_locs=[np.array([0, 0])])
                env.seed(idx)
                return env
            return _init
    ...
    for idx, traj in tqdm(enumerate(all_trajs), total=len(all_trajs)):
        ...
        if N < 16:
            continue
        new_obs = np.zeros((N, 6))
        for i in range(N):
            o = traj["observations"][i]
            a_xy = o["agent_ball"]
            b_xy = o["target_ball"]
            t_xy = o["target"]
            o = np.stack([t_xy, a_xy, b_xy]).flatten()
            new_obs[i] = o
    ...
    print(f"filtered out {len(all_trajs) - len(dataset['teacher'])} trajectories with length < 16")
    Path("datasets/boxpusher_v2").mkdir(parents=True, exist_ok=True)
    with open("datasets/boxpusher_v2/dataset_xirl.pkl", "wb") as f:
        pickle.dump(dataset, f)
    ids =sorted(list(dataset["teacher"].keys()))
    np.save("datasets/boxpusher_v2/dataset_xirl_train_ids.npy", ids)